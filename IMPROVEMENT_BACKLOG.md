...
Resolved (2026-07-09, owner decision): asked the project owner directly which of (a)/(b) to take. Chose (b) -- raised `JSON_PAYLOAD_BUDGET_BYTES` in `pipeline/audit.py` and spec Section 11.6's stated budget from 2MB to 5MB. No data trimmed, no lazy-loading added; committed payload (~3.5MB and growing slowly) comfortably fits with room for years of continued daily growth. (a) remains a legitimate future optimization for initial-paint latency but is a separate concern from the audit's payload-budget finding, which this closes.
...
Suggested fix (implemented, 2026-07-09 second `/improve` pass): verified by calling `BlockchainInfoChartsClient().fetch_chart('hash-rate')` live -- blockchain.info's own hash-rate chart is itself missing 2025-11-13 through 2025-11-15, confirming this is an upstream source gap, not a pipeline defect. Added to `pipeline/known_gaps.json` with that citation.
...
Suggested fix (implemented, 2026-07-09 second `/improve` pass): verified by calling `AlternativeMeFngClient().fetch_full_history()` live -- alternative.me's own Fear & Greed index is itself missing 2018-04-14 through 2018-04-16 (plausible, the index was brand new in early 2018), confirming an upstream source gap. The same live check also surfaced a second, previously-unreported fng_daily gap (2024-10-26) that the audit's "first gap" summary string had been silently masking since it only reports the earliest gap when count > 1. Both added to `pipeline/known_gaps.json` with citations.
...
Suggested fix (implemented, 2026-07-09 second `/improve` pass): see the P4 entry's resolution note -- project owner raised the budget to 5MB rather than trim history.
...
### [feature] Explain-everything info tooltips shipped (2026-07-09)
Source: manual (owner request)
Description: The build spec (`docs/BTC_ENGINE_ROOM_BUILD_SPEC.md` line 337) specified an "explain-everything tooltips" feature -- every metric label gets an info popover with a plain-English definition, why-it-matters, and source -- as the on-ramp for newcomers this project's whole positioning depends on. It was never actually built across any of the 6 completed phases; this closes that gap.
Suggested fix (implemented): 14 per-metric info-toggle button+panel pairs (masthead: price, blocks-to-halving, halving-eta, block height; 6 gauges; 4 projection charts) plus 2 section-level `<details>` glossaries (status-chip vocabulary under Gauges, the 6 audit checks nested inside the audit panel). New `assets/info.js` (a single delegated click handler, ~15 lines). `assets/style.css` gained `.info-toggle`/`.info-panel`/`.info-disclosure` plus layout adjustments (`.gauge-card h3` and a new `.chart-plate-heading` wrapper both converted to flex rows so the icon doesn't disturb existing alignment -- notably the chart cards' `.chart-plate-stats` "calibration plate" readout, which would have been pulled off the right edge by naively adding a third flex child). Existing `.chart-caption` text is unchanged -- the new panels are an additional friendlier layer above it, not a replacement. Reveal is an instant `hidden`-attribute toggle, no transition, so `prefers-reduced-motion` needed no special handling. One real DOM-lifecycle hazard caught before implementing: `live.js` overwrites `#odometer-caption`'s `textContent` on every block arrival, so the block-height info-toggle had to be placed as a sibling in a new `.odometer-caption-row` wrapper, not nested inside the caption itself, or it would have been wiped on the next block.

### [theme] Engine theme killed, Digital Rain made cinematic and the site's sole identity; Deviation Dial replaced with Market Sentiment (2026-07-09)
Source: manual (owner request, Fable director review)
Description: Owner requested killing the `engine` theme entirely (leaving Digital Rain as the site's only visual identity, no theme picker), making the rain "much more matrix like -- straight out of the movie," fixing a real readability bug (rain glyphs directly behind masthead/plate text with no contrast backing), and replacing the "Deviation Dial" projection with a Fear & Greed-based "Market Sentiment" chart. Both the theme kill and the escalated cinematic treatment reverse settled director calls from earlier the same day (CLAUDE.md Section 6 original rules 1/2/9) -- per the project's own process, a second Fable director review ran before implementation. Full ruling: `engine` cut approved with the credibility burden reassigned to strict data/chrome discipline (a new readability law: no DOM text ever over unbacked rain, ≥0.92 opacity backing, ≥0.96 for the masthead); a concrete cinematic-rain spec (variable per-column speed tiers, density 0.9, in-place glyph mutation, 3-step hot-head color gradient, rare "burst" columns, a block-arrival density/burst surge coupled to the same real new-block event driving the odometer and Block Rail, 20fps cap explicitly kept -- the choppiness is the aesthetic); an explicit IP fine/veto list (falling-glyph trope is generic and safe at any density/brightness; the film's title, quotes, proprietary glyph typeface, stills, and iconography are all hard vetoes -- public name stays "Digital Rain" only); and Deviation Dial approved for full removal from the pipeline (not just hidden -- publishing a number the site itself calls "a toy composite, not a valuation model" nowhere-shown would contradict the transparency ethos), replaced with a real, sourced-data "Market Sentiment" chart labeled honestly as sentiment, not valuation.
Suggested fix (implemented): `assets/rain.js` rewritten (speed tiers, density, mutation, hot-head gradient, burst columns, block-arrival surge via a new `ber:block` event `live.js` dispatches, rain on/off toggle API, canvas id renamed `matrix-rain-canvas` -> `digital-rain-canvas`); `assets/style.css` (engine token block deleted, matrix tokens promoted to `:root`, full-bleed masthead scrim, panel-backed `.plate`/footer/`.info-disclosure` per the new readability law, riveted corners retired, border-radius reduced project-wide for near-sharp terminal chrome, masthead H1 and `.plate` moved to IBM Plex Mono); `index.html` (theme switcher replaced with a Rain On/Off rocker, pre-paint theme script removed entirely, Archivo Black dropped from the Google Fonts URL, Deviation Dial card replaced with a Market Sentiment card); `assets/app.js` (dead theme code removed, new Rain toggle wiring, one-time H1 scramble-to-settle boot flourish, Latin caps+digits only, skipped under reduced-motion); `assets/live.js` (dead `isEngineTheme()` gating removed, new `ber:block` event dispatch); `assets/charts.js` (`renderDeviationDial` replaced with `renderMarketSentiment`, reads already-fetched `fng_daily` history, classification zones as dim `--panel-border`-opacity reference bands, never accent); `pipeline/fit_models.py`/`models.schema.json`/its tests (deviation dial computation, schema field, and tests removed entirely); `data/models.json` regenerated for real via `python -m pipeline.fit_models`. `engine` is not preserved in code -- recoverable from git history, same as the earlier `light` cut.

### [ux] End-to-end UI/UX audit: fixed 5 gaps, deferred 2 (2026-07-09)
Source: manual (owner request: "make sure each pixel makes full sense to the reader and needs to be there")
Description: audited every visible element against whether a reader could tell what it means and why it's there. Fixed: (1) header `.engine-status-lamp` named/read as if it summarized the "Engine Health" section further down when it's actually unrelated (WebSocket connectivity vs. daily-snapshot health) -- reworded its two live-text states to "live feed: connected"/"live feed: polling (WebSocket unavailable)" and added a disambiguating `title`; (2) `.block-rail` was the only masthead instrument with no touch/screen-reader-reachable explanation (native `title` only, `aria-hidden`) -- gave it the same caption-row + info-toggle pattern as the odometer; (3) Engine Health's 5 `.health-row`s reused the Gauges' `.status-chip` component but showed the raw source id as the label text instead of a status word, and had zero disclosure of their own -- chip now shows OK/STALE (source id demoted to a `title` hover detail) and a new `.info-disclosure` explains what a row means, explicitly distinguishing it from the Gauges above and noting why Fees/Mempool aren't listed; (4) the global "What do the status colors mean?" glossary never defined PASS/WARN/FAIL, only LIVE/DELAYED/STALE, even though the audit chip uses the former -- added one sentence to `#audit-glossary`'s intro. New `data-status="OK"` CSS added (green, alongside LIVE/PASS) since "OK" is deliberately not "LIVE" -- a once-a-day pipeline job succeeding isn't the same claim as a live stream being fresh, and conflating the two words would recreate the exact confusion this pass was fixing.
Considered and deferred (not implemented, flagged rather than silently dropped): (a) `.gauge-sub` (the secondary line under each of the 6 gauge values) does three unrelated jobs with identical styling across cards -- "committed <date>" metadata (hashrate/supply), a static descriptive label (fees/mempool), a second live datum (difficulty retarget %, fear&greed word) -- a reader can't tell which kind of information a given sub-line is from its appearance alone. Deferred because a good fix needs real visual sub-typing (an icon, a label prefix, or split into distinct slots), and doing that quickly risked adding clutter/failing the screensaver test rather than fixing the ambiguity. (b) Only the Power Law chart has a `.chart-controls` timeframe switcher (1Y/4Y/ALL); the other 3 charts have none, with no cue why. Deferred because this is arguably correct by design (Cycle Overlay is inherently "all cycles, normalized"; Mayer/200WMA and Market Sentiment are meant to be read as full history) rather than a real gap, and adding explanatory chrome to justify a non-issue would itself be more clutter -- worth revisiting only if a future reader genuinely asks why.
