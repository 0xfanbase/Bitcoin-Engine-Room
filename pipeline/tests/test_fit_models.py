...
def test_deviation_dial_averages_available_components():
    power_law = {"current": {"residual_percentile": 90.0}}
    mayer = {"current": {"percentile": 80.0}}
    cycle = {"current_epoch": {"cycle_percentile_vs_prior_epochs": 70.0}}

    result = fit_models.compute_deviation_dial(power_law, mayer, cycle)

    assert result["score_0_100"] == pytest.approx(80.0)
    assert result["label"] == "Redline"


def test_deviation_dial_low_score_is_idle():
    power_law = {"current": {"residual_percentile": 5.0}}
    mayer = {"current": {"percentile": 10.0}}
    cycle = {"current_epoch": {"cycle_percentile_vs_prior_epochs": 15.0}}
...
def test_deviation_dial_handles_missing_components():
    power_law = {"current": {"residual_percentile": 60.0}}
    mayer = {"current": None}
    cycle = {"current_epoch": None}

    result = fit_models.compute_deviation_dial(power_law, mayer, cycle)

    assert result["components"]["mayer_multiple_percentile"] is None
    assert result["score_0_100"] == pytest.approx(60.0)
